...
class SumoEnv(gym.Env):
    def __init__(self, sumocfg_path="Simulación/osm.sumocfg", rutas_dir="Simulación/rutas", max_steps=1800, modo_evaluacion=False):
        ...
        self.log_file = "estadisticas_DQN_alto.csv"
        self.episodio = 1

        if not self.modo_evaluacion:
    ...
    def step(self, action):
        ...
        stopped_vehicles = sum(traci.edge.getLastStepHaltingNumber(e) for e in self.edge_ids)
        reward = -stopped_vehicles

        self.total_queue_length += stopped_vehicles
        ...
        if terminated and not self.modo_evaluacion:
    ...
    def _get_obs(self):
        ns = traci.edge.getLastStepVehicleNumber(self.main_edges["norte"]) + \
             traci.edge.getLastStepVehicleNumber(self.main_edges["sur"])
        ew = traci.edge.getLastStepVehicleNumber(self.main_edges["oriente"]) + \
             traci.edge.getLastStepVehicleNumber(self.main_edges["occidente"])
        return np.array([ns, ew], dtype=np.float32)
