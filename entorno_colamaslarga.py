...
class SumoEnvColaMasLarga(gym.Env):
    def __init__(self, sumocfg_path="Simulación/osm.sumocfg", rutas_dir="Simulación/rutas", max_steps=1800):
        ...
        self.log_file = "estadisticas_colamaslarga_alto.csv"
        self.episodio = 1
    ...
    def step(self, _action=None):
        ...
        cola_ns = traci.edge.getLastStepHaltingNumber(self.main_edges["norte"]) + \
                  traci.edge.getLastStepHaltingNumber(self.main_edges["sur"])
        cola_ew = traci.edge.getLastStepHaltingNumber(self.main_edges["oriente"]) + \
                  traci.edge.getLastStepHaltingNumber(self.main_edges["occidente"])

        if self.time_in_current_phase >= self.min_green_time:
            if self.current_phase == 0 and cola_ew > cola_ns * self.urgency_factor and cola_ew >= self.queue_threshold:
                self._switch_phase(1)
            elif self.current_phase == 1 and cola_ns > cola_ew * self.urgency_factor and cola_ns >= self.queue_threshold:
        ...
        for veh_id in traci.vehicle.getIDList():
            ...
            self.vehiculos_unicos.add(veh_id)

            if veh_id in self.prev_speeds and self.prev_speeds[veh_id] > 0.1 and speed < 0.1:
                self.total_stops_real += 1
            self.prev_speeds[veh_id] = speed
        ...
        if done:
            ...
            paradas_prom_vehiculo = self.total_stops_real / veh_total if veh_total > 0 else 0
            throughput = (self.vehiculos_salidos * 3600) / self.step_count if self.step_count > 0 else 0
            with open(self.log_file, "a") as f:
                f.write(f"{self.episodio},{self.total_waiting_time},{self.total_queue_length},{veh_total},{self.total_stops_real},{tiempo_prom_sistema},{longitud_prom_cola},{paradas_prom_vehiculo},{throughput}\n")
            self.episodio += 1
    ...
    def _get_obs(self):
        return np.array([self.time_in_current_phase, self.current_phase], dtype=np.float32)
